...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/04/24 12:03:09 by mcassagn          #+#    #+#             */
...
int	ft_putnbru_fd(unsigned int n, int b, int fd)
{
	...
	{
		if ((n % b) < 10)
			return (ft_putnbru_fd((n / b), b, fd)
			+ ft_putchar_fd((n % b) + '0', fd));
		else
			return (ft_putnbru_fd((n / b), b, fd)
			+ ft_putchar_fd((n % b) - 10 + 'a', fd));
	}
}
...
int	ft_putpoint_fd(long unsigned int n, int fd)
{
	...
	{
		if (n < 10)
			return (ft_putstr_fd("0x", fd) + ft_putchar_fd(n + '0', fd));
		else
			return (ft_putstr_fd("0x", fd) + ft_putchar_fd(n - 10 + 'a', fd));
	}
	else
	{
		if ((n % 16) < 10)
			return (ft_putpoint_fd(n / 16, fd)
